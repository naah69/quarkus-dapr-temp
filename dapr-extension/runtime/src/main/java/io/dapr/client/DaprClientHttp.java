...
public class DaprClientHttp extends AbstractDaprClient {
    ...
    public static final TypeRef<DaprHttp.Response> RESPONSE_TYPE = TypeRef.get(DaprHttp.Response.class);
    ...
    private <T> Mono<T> getMono(TypeRef<T> type, DaprHttp.Response r) {
        try {
            if (!Objects.equals(RESPONSE_TYPE, type)) {
    ...
    public Mono<Void> deleteState(DeleteStateRequest request) {
        try {
            ...
            if (etag != null && !etag.trim().isEmpty()) {
                headers.add(HEADER_HTTP_ETAG_ID, etag);
            }
