...
public class DaprHttp implements AutoCloseable {
     ...
     */
    private static final Set<String> ALLOWED_CONTEXT_IN_HEADERS = Collections
            .unmodifiableSet(new HashSet<>(Arrays.asList("grpc-trace-bin", "traceparent", "tracestate")));
    ...
    public static class Response {
         ...
         */
        public Response(byte[] body, Map<String, List<String>> headers, int statusCode) {
    ...
    private CompletableFuture<Response> doInvokeApi(String method,
        ...
        String contentType = headers != null && headers.get(Metadata.CONTENT_TYPE) != null
                ? headers.get(Metadata.CONTENT_TYPE).get(0)
                : null;
        ...
        if (content == null) {
            body = mediaType.equals(MEDIA_TYPE_APPLICATION_JSON)
                    ...
                    : RequestBody.create(mediaType, new byte[0]);
        } else {
        ...
        Optional.ofNullable(urlParameters).orElse(Collections.emptyMap()).entrySet().stream()
                .forEach(urlParameter -> Optional.ofNullable(urlParameter.getValue()).orElse(Collections.emptyList()).stream()
                        .forEach(urlParameterValue -> urlBuilder.addQueryParameter(urlParameter.getKey(), urlParameterValue)));
        ...
        if (headers != null) {
            Optional.ofNullable(headers.entrySet()).orElse(Collections.emptySet()).stream()
                    .forEach(header -> header.getValue().forEach(v -> requestBuilder.addHeader(header.getKey(), v)));
        }
    ...
    private static class ResponseFutureCallback implements Callback {
        ...
        public void onResponse(Call call, okhttp3.Response response) throws IOException {
            ...
            future.complete(new Response(result, headers, response.code()));
        }
    }
